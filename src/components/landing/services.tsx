...
const WebService = ({ proposalData }: { proposalData: ProposalData }) => {
  ...
  return (
    <div ref={sectionRef} className="service-card hyperglass rounded-[2.5rem] overflow-hidden reveal">
...
const SocialService = ({ proposalData }: { proposalData: ProposalData }) => {
  const service = proposalData.bloques.find((b) => b.id === 'servicio_social') as any;
  const sectionRef = useRef<HTMLDivElement>(null);
  ...
  const techIncludes = service.caracteristicas.find(
    (c: any) => c.titulo === 'Entregables Mensuales' || c.titulo === 'Incluye'
  );

  const noIncluye = service.caracteristicas.find(
    (c: any) => c.titulo === 'No incluye (Adicionales)' || c.titulo === 'No incluye'
  );
      ...
      <div className="p-8 md:p-14">
        <div className="flex flex-col md:flex-row justify-between items-start mb-12 gap-6">
          <div>
            ...
            <h3 className="text-4xl font-outfit font-bold mt-4 text-slate-900">
              {service.titulo.split('·')[1]?.trim() || service.titulo}
            </h3>
            <p className="text-blue-600 font-medium mt-2">
              {service.bullet_principal}
            </p>
...
export default function Services({ proposalData }: { proposalData: ProposalData }) {
  ...
  const socialServiceData = proposalData.bloques.find(b => b.id === 'servicio_social');
  ...
  return (
    <>
      <div className="space-y-24">
        ...
        {socialServiceData && <SocialService proposalData={proposalData} />}
      </div>
    </>
  );
}
